...
\begin{algorithm}[H]
    ...
    \For{$ i \in \{0, 1, 2, \cdots, v-1 \}$}
        \State{$\matr{y}_{train}  \leftarrow \{\matr{y}_i \}_{i = 1, 2, \cdots, s - 1 + i}$ Train on all information before the validation point.}
        \State{$\matr{y}_{val}    \leftarrow \matr{y}_{ s + i}$ This is the validation instance in $\matr{y}$.}
        \State{$\matr{X}_{train}  \leftarrow \{\matr{X}_i \}_{i = 1, 2, \cdots, s - 1 + i}$ Train on all information before the validation point.}
        \State{$\matr{X}_{val}    \leftarrow \matr{X}_{ s + i}$ This is the validation instance in $\matr{X}$.}
        \State{Do training: $\theta(\phi_0)_0 = \arg_{\theta(\phi_0)} \max \mathcal{E}(\widehat{\matr{y}_{train}}, \matr{y}_{train}), \quad \widehat{\matr{y}_{train}} = f(\theta(\phi_0); \matr{X}_{train})$}
        \State{Store the fitness score of $\theta(\phi_0)_0$ in list $L$, i.e., $\mathcal{E}(f(\theta(\phi_0)_0; \matr{X}_{train}), \matr{y}_{train})$}
    \EndFor
    \State{Let $V_{\phi_0} \equiv \frac{1}{v}\sum{L}$ be the mean of the scores stored in list $L$.}
    \end{algorithmic}
\end{algorithm}
The result for validating a single hyperparameter $\phi_0$ is the validation score $V_{\phi_0}$ we compute in the final step of the algorithm. Then if we have a set of $k$ candidate hyperparameters $\theta = \{ \theta_i \}_{i = 1, 2, \cdots, k}, \; k \in \mathbb{N}$, we can repeat such validation process $k$ times and yield $k$ validation scores $\{ V_{\phi_i} \}, \; \forall i \in \{1, 2, \cdots, k \}$. We then take the hyperparameter set with the most preferable fitness score
\begin{equation*} 
