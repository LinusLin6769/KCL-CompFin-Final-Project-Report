...
\subsection{Machine Learning Regression Modelling}\label{subsec: ML modelling}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
    \State{Let a validation measure $\mathcal{V}$ be given.}
    ...
    \For{$ i \in \{0, 1, 2, \cdots, v-1 \}$}
        ...
        \State{Do training: $\theta(\phi_0)_0 \leftarrow \arg_{\theta(\phi_0)} \min \mathcal{E}(\widehat{\matr{y}}_{train}, \matr{y}_{train}), \; \widehat{\matr{y}}_{train} = f(\theta(\phi_0); \matr{X}_{train})$}
        ...
        \State{Compute validation score for the $\theta(\phi_0)_0$: $\nu \leftarrow \mathcal{V}(\widehat{\matr{y}}_{val}, \matr{y}_{val})$}
        \State{Store the validation score $\nu$ in list $L$}
    \EndFor
